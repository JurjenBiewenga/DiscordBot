...
namespace DiscordBot.Services
{
    public static class Config
    {
        ...
        public static void SaveData(string saveName = configName)
        {
            IO?.Save(data, saveName);
        }

        public static void LoadData(string saveName = configName)
        {
            if (IO != null)
                data = IO.Load(saveName);
        }
        ...
        private static void SetValue(Dictionary<string, object> dict, string[] keys, object value, int currIndex)
        {
            if (dict == null)
            {
                Console.WriteLine("Failed to set a value in the config");
                return;
            }
